...
class GameManager {
    ...
    /**
     * Creates a special room for a challenge game
     * @param {string} code - Room code generated by ChallengeManager
    ...
    createChallengeRoom(code, challengeData) {
        ...
        const room = {
            code: code,
            isChallenge: true, // Special flag for challenge rooms
            players: [], // Start empty, players join immediately after
            ...
            gameState: {
                scores: [0, 0],
                currentPlayer: 1,
                ballPosition: fieldConfig ? { ...fieldConfig.ballStartPosition } : null
            },
            timeoutTimer: setTimeout(() => this.removeRoom(code), ROOM_TIMEOUT),
            createdAt: Date.now()
        };
        ...
        this.rooms.set(code, room);
        console.log(`[INFO] Meydan Okuma Odası oluşturuldu: ${code}`);
        return room;
    }
    ...
    joinRoom(ws, roomCode, playerName) {
        ...
        // --- Challenge Room Join Logic ---
        if (room.isChallenge) {
            if (room.state !== 'waiting_challenge') {
            ...
            const playerId = room.players.length + 1; // 1 for first player, 2 for second
            const player = {
                ws,
                name: playerName || `Oyuncu ${playerId}`,
                id: playerId,
                connected: true,
                disconnectTimer: null
            };
            ...
            console.log(`[INFO] Challenge oyuncusu katıldı: ${roomCode} - P${playerId} (${playerName})`);

            if (room.players.length === 2) {
                // Both players joined, start the game immediately
                ...
                const startData = {
                    ...
                    goalkeeperEnabled: room.settings.goalkeeperEnabled,
                    goalkeeperSize: room.settings.goalkeeperSize
                };
